...
class EmblemController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        if (config('athena.allow_emblem_upload_on_woe') === true) {
            $woe_status = DB::connection($request->world_name)->table(Utils::tb('web_service'))->where('key', 'woe')->first();
        ...
        $old_data = DB::connection($request->world_name)->table(Utils::tb(self::TABLE))->where('guild_id', $request->guild_id)->where('world_name', $request->world_name)->first();
        $file_name = $file->store(self::STORAGE_DIRECTORY);
        $version = 0;
        if ($old_data) {
            Storage::delete($old_data->file_name);
            $version = $old_data->version + 1;
            DB::connection($request->world_name)->table(Utils::tb(self::TABLE))->where('guild_id', $request->guild_id)->where('world_name', $request->world_name)
                ->update([
                    'file_name' => $file_name,
                    'version' => $version,
                ]);
    ...
    public function download(Request $request)
    {
        ...
        $emblem = DB::connection($request->world_name)->table(Utils::tb(self::TABLE))->where('guild_id', $request->guild_id)->where('world_name', $request->world_name)->first();
        if ($emblem) {
            if (true || $emblem->version > $version) {
                return Storage::download($emblem->file_name); // TODO: Support versioning
            }
